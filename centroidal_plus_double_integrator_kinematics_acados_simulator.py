...
class CentroidalPlusLegKinematicsAcadosSimulator:
    ...
    def __init__(self, model, solver):
        ...
        # create integrator
        self.acados_integrator = AcadosSimSolver(acados_simulator)
    ...
    def count_constraint_violations(self, x_sim, WITH_VISUALIZATION=False):
        ...
        for sim in range(x_sim.shape[0]):
            print("starting simulation nb: ", sim, ".....")
            VIOLATED = False
            for time_idx in range(x_sim.shape[1]-1):
                ...
                if VIOLATED:
                    break        
        return nb_contact_location_constraint_violations


    def simulate(self, x0, u, nb_sims=1, MPC=False, WITH_DISTURBANCES=False, STOCHASTIC=False):
        ...
        for sim in range(nb_sims):
            ...
            for time_idx in range(N-1):
                ...
                if SAMPLE_DISTURBANCE:
                    w = self.sample_addtitive_uncertainties()
                ...
                acados_integrator.set("u", u[time_idx, :])
                # simulate system
                status = self.acados_integrator.solve()
                if status != 0:
                    raise Exception('acados returned status {}. Exiting.'.format(status))
                # integrate closed-loop state
...
if __name__ == "__main__":
    ...
    model = CentroidalPlusLegKinematicsCasadiModel(conf, STOCHASTIC_OCP=True)
    solver = CentroidalPlusLegKinematicsAcadosSolver(
        model, x_warmstart, u_warmstart, MPC=False
        )
    x_sol, u_sol = solver.solve()
    simulator = CentroidalPlusLegKinematicsAcadosSimulator(model)
    x_sim = simulator.simulate(x_sol[0, :], u_sol, nb_sims=5, WITH_DISTURBANCES=True)
